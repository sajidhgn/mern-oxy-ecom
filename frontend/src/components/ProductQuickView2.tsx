...
const ProductQuickView2: FC<ProductQuickView2Props> = ({ className = "" }) => {
  ...
  const renderStatus = () => {
    ...
    const CLASSES =
      "absolute top-3 left-3 px-2.5 py-1.5 text-xs bg-white dark:bg-slate-900 ox-shadow-lg rounded-full flex items-center justify-center text-slate-700 text-slate-900 dark:text-slate-300";
    if (status === "New in") {
      return (
  ...
  return (
    <div className={`ox-ProductQuickView2 ${className}`}>
      {/* MAIn */}
