...
const ProductCard: FC<ProductCardProps> = ({
      ...
      <div className="absolute bottom-0 inset-x-1 space-x-1.5 rtl:space-x-reverse flex justify-center opacity-0 invisible group-hover:bottom-4 group-hover:opacity-100 group-hover:visible transition-all">
        {sizes.map((size, index) => {
          return (
            <div
              key={index}
              className="ox-shadow-lg w-10 h-10 rounded-xl bg-white hover:bg-slate-900 hover:text-white transition-colors cursor-pointer flex items-center justify-center uppercase font-semibold tracking-tight text-sm text-slate-900"
  ...
  return (
    <>
      <div
        className={`ox-ProductCard relative flex flex-col bg-transparent border border-gray-200 group hover:shadow-xl rounded-lg ${className}`}
      >
        ...
        <div className="space-y-4 px-2.5 pt-5 pb-2.5">
          {/* {renderVariants()} */}
          <div>
            <h2 className="ox-ProductCard__title text-base font-semibold transition-colors">
              {name}
            </h2>
